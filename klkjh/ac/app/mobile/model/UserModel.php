...
class UserModel extends Model
{
     ...
     * @return int
     */
    public function namePasswordReset($user_login, $password)
    {
        $result = $this->where('user_login', $user_login)->find();
        if (!empty($result)) {
            $data = [
                'user_pass' => cmf_password($password),
            ];
            $this->where('user_login', $user_login)->update($data);
            return 0;
        }
        return 1;
    }
    ...
    public function editData($user)
    {
        ...
        if ($this->allowField('user_nickname,sex,signature,avatar')->save($user, ['id' => $userId])) {
    ...
    public function editPassword($user)
    {
        ...
        if (!cmf_compare_password($user['old_user_pass'], $pass['user_pass'])) {
            return 1;
        }
        $data['user_pass'] = cmf_password($user['user_pass']);
