...
$.fn.photoClip = function(option) {
	if (!window.FileReader) {
		webToast("您的浏览器不支持 HTML5 的 FileReader API， 因此无法初始化图片裁剪插件，请更换最新的浏览器！","bottom", 1000);
		return;
	}
...
function photoClip(container, option) {
	...
	$file.change(function() {
		...
		if (!/image\/\w+/.test(this.files[0].type)) {
			webToast("图片格式不正确，请选择正确格式的图片文件！","bottom", 1000);
			return false;
		} else {
	...
	function clipImg() {
		if (!imgLoaded) {	
			webToast("亲，当前没有图片可以裁剪！","bottom", 1000);
			return;
		}
