var played={}, mysql=require('mysql'),wjLiRunLv=0,
http=require('http'),
https=require('https'),
requestHttp,
url=require('url'),
...
calc=require('./parse/parse-calc-time.js'),
exec=require('child_process').exec,
execPath=process.argv.join(" "),
//onparse=require('./kj-data/parse-string.js'),
parse=require('./parse/parse-calc-count.js');
...
var timers={};		// 任务记时器列表
var encrypt_key='301985f3cbf93e46ecc8248e0771292d';
...
function getPlayedFun(cb){
	...
	client.query("select id, rule_fun from ac_played", function(err, data){
		if(err){
			log('读取玩法配置出错：'+err.message);
		}else{
			data.forEach(function(v){
				played[v.id]=v.rule_fun;
			});
...
function run(conf){
	...
	requestHttp.request(option, function(res){
...
function submitData(data, conf){
	...
	//alert(110);
	client.query("insert into ac_data_"+data.table+"(time, number, data) values(?,?,?)", [data.time, data.number, data.data], function(err, result){
		//console.log(err);
		if(err){
			//console.log(err);
			// 普通出错
...
function calcJ(data){
	...
	sql="select * from ac_plan_"+data.table+" where flag=0 and number='' and from_number <="+number+" and to_number >="+number;	
	
	client.query(sql, function(err, plans){
		if(err){
			//console.log(data);
			//console.log(err.sql);
			log("读取投注出错："+err);
		}else{
			var sql, fun, zjCount, sqls=[];
			
			plans.forEach(function(plan){				
				try{
					fun=parse[played[plan.played_id]];
					//console.log(played[plan.playedId]);
					if(typeof fun!='function') throw new Error('算法不是可用的函数');
				}catch(err){
					log('计算玩法[%f]中奖号码算法不可用：%s'.format(plan.played_id, err.message));
					return;
				}
				
				try{
					zjCount=fun(plan.data, data.data)||0;
				}catch(err){
					log('计算中奖号码时出错：'+err);
					return;
				}	
				...
				sqls.push(client.format(sql, [data.table, plan.id, zjCount, number, plan.to_number, data.data, 'ssc-'+encrypt_key]));
			});
