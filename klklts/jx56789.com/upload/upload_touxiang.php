...
$attachDir='../face/p1';//上传文件保存路径，结尾不要带/
$dirType=1;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
$maxAttachSize=20971520;//最大上传大小，默认是2M
$upExt='jpg,gif,png';//上传扩展名
...
$imagewidth='';
 $imageheight='';
$tempPath=$attachDir.'/'.$_SESSION['login_uid'].'.tmp';
$localName='';
...
if(isset($_SERVER['HTTP_CONTENT_DISPOSITION'])&&preg_match('/attachment;\s+name="(.+?)";\s+filename="(.+?)"/i',$_SERVER['HTTP_CONTENT_DISPOSITION'],$info)){//HTML5上传
	...
	elseif(empty($upfile['tmp_name']) || $upfile['tmp_name'] == 'none')$err = '无文件上传';
               ...
               $imagesize=getimagesize($upfile['tmp_name']); 
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			$newFilename=$_SESSION['login_uid'].'.png';
			$targetPath = $attachDir.'/'.$newFilename;
			
			rename($tempPath,$targetPath);
                        //转化为缩略图
                        require_once '../include/common.inc.php';
                         mkThumbnail($targetPath, 200, 200,$targetPath); 
			...
			else $msg="{'url':'".$targetPath."','localname':'".jsonString($localName)."','info':'{$_POST[info]}','imagewidth':'{$imagewidth}','imageheight':'{$imageheight}'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
