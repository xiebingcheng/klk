...
SysSend=(function(){
	return{
		...
		msg:function(){
			...
			var str = '{"type":"SendMsg","ToChatId":"'+ToUser.id+'","IsPersonal":"'+getId('Personal').value+'","Style":"","tanmu":"'+tanmuSelect+'","Txt":"'+msgid+'_+_'+Msg+'"}';
		ws.send(str);
		...
		command:function(cmd,value){
			...
			{	
				...
				//var str='SendMsg=M='+touser+'|'+IsPersonal+'|'+Style+'|'+Msg;
                               var str = '{"type":"SendMsg","ToChatId":"'+touser+'","IsPersonal":"'+IsPersonal+'","Style":"","tanmu":"false","Txt":"'+Msg+'"}'; 
				ws.send(str);
				getId('Msg').innerHTML='';
				getId('Msg').focus();
			}
...
function FormatMsg(Msg)
{
		...
		switch(command[1])
		{
			...
			case 'rebotmsg':
	            ...
	            if(Msg.tanmu=='true'){
	            	...
	            	sendBarrage(decodeURIComponent(command[3])); 
	            };
			break;		
			case 'kick':
				if(My.chatid==toUser.chatid){				
 ...
 var    divHtml = $('<div class="dShow positionAbs"><span style="text-shadow:0 0 10px '+rollarr[1]+'">'+rollarr[0]+'</span></div>');$("body").append(divHtml), $(".dShow").find("span").css({
	...
	if(toUser.chatid!="ALL"){
		...
		str='<div style="clear:both; height:0px;"></div><div class="msg'+who+'" id="'+msgid+'" '+msgAuditShow+'><div class="msg_head"><img src="'+grouparr[User.color].ico+'" class="msg_group_ico" title="'+grouparr[User.color].title+"-"+grouparr[User.color].sn+'"></div><div class="msg_content"><div>'+IsPersonal+'<font class="u" style="color:'+aColor[User.sex]+'" onclick="UserList.menu_in_records(\''+User.chatid+'\',$(this))">'+User.nick+'</font>&nbsp;&nbsp;<font class="date">'+date+'</font></div><div class="layim_chatsay" ><font>'+Txt+'</font></div>'+msgBlockBt+msgAuditBt+'</div></div><div style="clear:both; height:0px;"></div>';
...
function OnResize(){
	...
	if(cw>=1280){
		if(cw>=1280)vw=cw-742;
                if(cw>=1366)vw=cw-792;
		if(cw>=1440)vw=cw-760;
		if(cw>=1600)vw=cw-800;
                if(cw>=1920)vw=cw-950;
					
			$('#UI_Central').css("margin-left",vw+252);
                      var _mLeft = parseInt($("#UI_Left").css("margin-left"));
	     _mLeft < 0?$('#UI_Right').css("width",vw+244):$('#UI_Right').css("width",vw);
	...
	// 中间内容
	$('#UI_Right').css("height",cH);

	$('#NoticeList').height($('.NoticeList').width()*0.2);
    $('.ck-slide').height($('.NoticeList').width()*0.2);
    // $('.gift-img-left').width($('.NoticeList').width()*0.4);
	$('#OnLine_MV').css("height",cH-$('#UI_Head').height()-100 - $('#NoticeList').height());
	$('#mvMask').css("height",$('#OnLine_MV').height() - 37);
