	...
	return{
       ...
       skin: 'nuoyun-layim',
		...
		send:function(){
			...
			if(typeof(toUserInfo)=="undefined"||user.chatid.indexOf('x_r')>-1){layer.msg('用户不在线,消息转存为离线消息！',{icon: 6,shift: 6,time:1000});}
			var msg=encodeURIComponent($("#layim_write").html().str_replace().replace("<br>",""));
			//var str='SendMsg=M='+user.chatid+'|true|color:#000|'+msg;
                        if($.trim(msg)=='')return;
			 var str = '{"type":"SendMsg","ToChatId":"'+user.chatid+'","IsPersonal":"true","Style":"","tanmu":"false","device":"computer","Txt":"'+msg+'"}';
	                ws.send(str);
		...
		newtab:function(u){
			...
			if(win.find('#layim_user'+u.chatid).length<1){
				layim_chatmore.find('ul>li').removeClass('layim_chatnow');
       			layim_chatmore.find('ul').append('<li data-qq="" data-id="'+ u.chatid +'" data-nick="'+u.nick+'" id="layim_user' +u.chatid +'" class="layim_chatnow"><img src="' +u.touxiang +'"><span><b class="layim_msgnum">0</b>'+ u.nick +'</span><em  data-id="'+ u.chatid +'">×</em></li>');
		...
		showtab:function(u){
			...
			win.find('.layim_face>img').attr('src', u.touxiang);
...
UserList=(function(){
	...
	return{
        ...
        delmyuser:function(id){
			...
			} else if(My.color=='3'&&u.vip==My.name){
				$("#group_myuser").find("#myuser"+id).remove();
				onlinmyuser--;
				OnlineOtherNum.innerHTML=onlinmyuser;
			}
		...
		add:function(id,u){
			...
			list[id]=u;
			 //else {
	            ...
	            var iscam='';
	            if(My.qx=='1') {
					iscam='<span class="vipico"><img src="/images/shutup.png" border="0" class="state shut-up"/>'+vip_ico+'</span>';
	            } else {
	                iscam='<span class="vipico">'+vip_ico+'</span>';    
	            }
				li.innerHTML = '<a href="javascript:void(0)"><font style="color:#FFF">'
				 				+iscam
				...
				li.ondblclick=function() { 
		            if (u.color == 2 || u.color == 17) {
		                ...
		                if(!check_auth("msg_priv")) {
		                	layer.msg('没有私聊权限！',{icon: 5,shift: 6,time:1000});
		                    return;
		                }
		               $.ajax({
                                type: 'get',
                                dataType: 'json',
                                url: '../ajax.php?act=check_auth&app=msg_priv',
                                success: function (data) {
                                    if (data.state == "yes") {
                                if(u.chatid!=My.chatid&&u.chatid.indexOf('x_r')<0) {
		                    POPChat.newtab(u);POPChat.showtab(u);
	            ...
	            if(u.age==1 && My.qx=='1') {
	                $('#' + u.chatid + ' a font span .shut-up').show();    
				}
			//}
			if (u.color == 2 || u.color == 3 || u.color == 4) {
				UserList.addmyuser(u);
			}
		},
		...
		menu_in_records:function(chatid, target)
		{ 
			var u=UserList.get(chatid);
			if (u == undefined) {
				return false;
			}
            var xx=0;
			...
			{
				UserMenu.add('Manage_view.png','查看资料',function() {
					getId('menu').style.display='none';
					if(!check_auth("user_info")) {
						layer.msg('没有用户资料查看权限！',{shift: 6});return;}
						openWin(2,false,'room/profiles.php?uid='+u.chatid,460,630);
				});
                xx=xx+27;
				if(check_auth("room_admin")) {
					UserMenu.hr();
                    UserMenu.add('Manage_Sayto.png','对TA私聊',function() {
                    	getId('menu').style.display='none';
                    	if(!check_auth("msg_priv")) {
                    		layer.msg('没有私聊权限！',{shift: 6});return;
                    	}
                             $.ajax({
                                type: 'get',
                                dataType: 'json',
                    ...
                    xx=xx+27;
					UserMenu.hr();
					UserMenu.add('Manage_Silent.png','禁言',function() {
						//ToUser.set(u.chatid,u.nick);
                                                getId('menu').style.display='none';
						SysSend.BanSpeak(u.chatid);
					});
                    xx=xx+27;
                    UserMenu.hr();
					UserMenu.add('Manage_Kick.png','踢出、封IP',function() {
						getId('menu').style.display='none';
						if(!check_auth("user_kick")) {
							layer.msg('没有用户踢出权限！',{shift: 6});
							return;
						}
						UKick.ShowMb(u);
					});
                    xx=xx+27;
                    UserMenu.hr();
					UserMenu.add('Manage_Sayto.png','对TA说',function() {
						getId('menu').style.display='none';
						if(!check_auth("user_kick")) {
							layer.msg('没有对TA说话的权限！',{shift: 6});
							return;
						}
						ToUser.set(u.chatid, u.nick)
					});
			...
			$(target[0].parentElement.parentElement).append(UserMenu.MObj);
			UserMenu.display(0, -xx-27);
		},
		menu:function(u)
		{ 
                    var xx=0;
			...
			UserMenu.add('Manage_view.png','查看资料',function(){getId('menu').style.display='none';if(!check_auth("user_info")){layer.msg('没有用户资料查看权限！',{shift: 6});return;}openWin(2,false,'room/profiles.php?uid='+u.chatid,460,630);});
                        xx=xx+27;
				if(check_auth("room_admin")) {
					if (u.chatid.indexOf('x_r') > -1) {
	                    UserMenu.hr();
						UserMenu.add('Manage_UnSilent.png','发言',function() {
							getId('menu').style.display='none';
							var $_option = $('<option value="' + u.chatid + '" selected data-id="' + u.chatid + '">发言人-' + u.nick + '</option>');
							$('#chat_type').append($_option);
							$_option.siblings().removeAttr("selected");
						});
	                    xx=xx+27;
					}
					UserMenu.hr();
                                      UserMenu.add('Manage_Sayto.png','对TA私聊',function(){getId('menu').style.display='none';
                                         if (u.color == 2 || u.color == 17) {
				POPChat.newtab(u);POPChat.showtab(u);
	            	}else{
                            if(!check_auth("msg_priv")){
                           ...
                           if(u.chatid!=My.chatid&&u.chatid.indexOf('x_r')<0){
                                POPChat.newtab(u);POPChat.showtab(u);
                                       ...
                                       xx=xx+27;
					UserMenu.hr();
					UserMenu.add('Manage_Silent.png','禁言',function(){getId('menu').style.display='none';SysSend.BanSpeak(u.chatid);});
                                        xx=xx+27;
                                        UserMenu.hr();
...
SysSend=(function(){
	return{
		...
		msg:function(){
                        if(RoomInfo.banspeak==1 && My.color != 2){layer.msg('全体禁言中！',{icon: 7,shift: 6});return false;}
                        if(My.banspeak==1){layer.msg('你被禁言了',{icon: 7,shift: 6});return false;}
			if(!check_auth("msg_send")){layer.msg('没有发言权限！',{icon: 7,shift: 6});return false;}
			/*判断是否是机器人*/
			if($('#chat_type option:selected').data('id').indexOf('x_r') > -1) {
				SysSend.command('rebotmsg',encodeURIComponent(getId('Msg').innerHTML));
				return true;
			}
		    ...
		    var Msg=encodeURIComponent($.trim(Msg.str_replace_msg()));
			if(Msg==oldMsg && My.qx=='0'){layer.msg('请勿刷屏！', {icon:7,shift: 6});getId('Msg').innerHTML='';return;}
			var newTime=new Date().getTime();
			if(newTime-oldTime<500){layer.msg('说话速度过快~歇会儿！', {icon:7,shift: 6});return;}
			if(msgTag){if(newTime-oldTime>5000)msgTag=false;else {layer.msg('说话速度过快~歇会儿！', {icon:7,shift: 6});return;}}
                        if(My.color=="0"){
			...
			if(Msg!='') {  
				...
				var str = '{"type":"SendMsg","ToChatId":"'+ToUser.id+'","IsPersonal":"'+getId('Personal').value+'","Style":"","device":"computer","tanmu":"'+tanmuSelect+'","Txt":"'+msgid+'_+_'+Msg+'_+_'+containKey + '"}';
				ws.send(str);
				oldMsg=Msg;
				oldTime=new Date().getTime();
				getId('Msg').innerHTML='';
				getId('Msg').focus();
				PutMessage(My.rid,My.chatid,ToUser.id,My.nick,toUserInfo.nick,getId('Personal').value,'',Msg,msgid,tanmuSelect);
            }
			return true;
		},
		command:function(cmd,value){
			...
			switch(cmd)
			{
				...
				case 'rebotmsg':
                    // if(ToUser.id.indexOf('x_r')<0){layer.msg('该用户不是机器人！',{shift: 6});return false;}
                                    var msgid=randStr()+randStr();
                    var rebotChatId = $('#chat_type option:selected').data('id');
					if (rebotChatId == ToUser.id) {
						ToUser.set('ALL','大家');
					}
					Msg+='C0MMAND_+_'+cmd+'_+_'+rebotChatId+'_+_'+ToUser.id+'_+_'+value+'_+_'+msgid;
					var rebotInfo=UserList.get(rebotChatId);
					IsPersonal='false';
					PutMessage(My.rid,rebotInfo.chatid,ToUser.id,rebotInfo.nick,ToUser.name,rebotInfo.color,'font-size:13px',value,msgid);
				break;
				default:
					Msg+='C0MMAND_+_'+cmd+'_+_'+value;
					IsPersonal='false';
			...
			{	
				...
				//var str='SendMsg=M='+touser+'|'+IsPersonal+'|'+Style+'|'+Msg;
                               var str = '{"type":"SendMsg","ToChatId":"'+touser+'","IsPersonal":"'+IsPersonal+'","Style":"","device":"computer","tanmu":"false","Txt":"'+Msg+'"}'; 
				ws.send(str);
				getId('Msg').innerHTML='';
				getId('Msg').focus();
			}
		},
                BanSpeak:function(touser)
		{ 
                   $.ajax({
                    type: 'get',
                    dataType:'json',
                    url: '../ajax.php?act=check_auth&app=ban_speak',
                    success:function(data){
                        if(data.state=="yes"){ 
                            if ($('#OnLineUser_FindList').is(":hidden")) {
                                if ($('#' + touser + ' a font span .shut-up').is(":hidden")) {
                                    $('#' + touser + ' a font span .shut-up').show();
                                    var str = '{"type":"SendMsg","ToChatId":"' + touser + '","IsPersonal":"false","Style":"","device":"computer","tanmu":"false","Txt":"C0MMAND_+_send_Msgblock_+_1"}';

                                } else {
                                    $('#' + touser + ' a font span .shut-up').hide();
                                    var str = '{"type":"SendMsg","ToChatId":"' + touser + '","IsPersonal":"false","Style":"","device":"computer","tanmu":"false","Txt":"C0MMAND_+_send_Msgblock_+_0"}';

                                }

                            }else{
                                  if ($('#fn' + touser + ' a font span .shut-up').is(":hidden")) {
                                    $('#' + touser + ' a font span .shut-up').show();
                                    $('#fn' + touser + ' a font span .shut-up').show();
                                    var str = '{"type":"SendMsg","ToChatId":"' + touser + '","IsPersonal":"false","Style":"","device":"computer","tanmu":"false","Txt":"C0MMAND_+_send_Msgblock_+_1"}';

                                } else {
                                    $('#' + touser + ' a font span .shut-up').hide();
                                     $('#fn' + touser + ' a font span .shut-up').hide();
                                    var str = '{"type":"SendMsg","ToChatId":"' + touser + '","IsPersonal":"false","Style":"","device":"computer","tanmu":"false","Txt":"C0MMAND_+_send_Msgblock_+_0"}';
		...
		case "UMsg":
			...
			if(Msg.UMsg.IsPersonal!='true'){
				MsgShow(str,type);
                    ...
                    sendBarrage(decodeURIComponent(str[1]));  
                    };
			}
			else
			{
			  if(Msg.UMsg.ToChatId==My.chatid){
                    ...
                    POPChat.showmsg(UserList.get(Msg.UMsg.ChatId),UserList.get(Msg.UMsg.ToChatId),"<font>"+decodeURIComponent(Msg.UMsg.Txt.str_replace())+"</font>");
                }
...
String.prototype.str_replace=function(t){
	...
	return str;
        ...
        //聊天信息过滤
        String.prototype.str_replace_msg=function(t){
	var str=this;

	str= str.replace(/<\/?(?!br|img)[^>]*>/ig,'').replace(/\r?\n/ig,' ').replace(/(&nbsp;)+/ig," ").replace(/(=M=)+/ig,"").replace(/(|)+/ig,"").replace(/(SendMsg)/ig,'');
...
function FormatMsg(Msg)
{
		...
		switch(command[1])
		{
			...
			break;
				...
				var giftid = command[5],
					...
					giftname = command[7],
					jid = command[9],
					jusername = command[10],
					from_client_name = command[8],
					cover = 'room/images/gift/gift_bg.png',
					logo = command[4];
				limit();
				if ($('.div' + giftid + userid + jid).length > 0) {
					var num = parseInt(sessionStorage.getItem(giftid + userid + jid)) + 1;
					sessionStorage.setItem(giftid + userid + jid, num);
					$('.div' + giftid + userid + jid).find('i').html('<em style="background:url(room/images/gift/texthxpic.png)"></em>'+
					...
					var $gifuser = $('.div'+giftid+userid+jid).find('i')[0];
					...
					new_obj = $("<div class='fly_huang putonggift div"+giftid+userid+jid+
					...
					rank();
					var $gifuser_ii = $('.div'+giftid+userid+jid).find('i')[0];
			...
			case 'rebotmsg':
	            ...
	            if(Msg.tanmu=='true'){
	            	// writeBarrage(decodeURIComponent(command[3]))
	            	sendBarrage(decodeURIComponent(command[4])); 
	            };
			break;		
			case 'kick':
				if(My.chatid==toUser.chatid){				
 ...
 var    divHtml = $('<div class="dShow positionAbs"><span style="text-shadow:0 0 10px '+rollarr[1]+'">'+rollarr[0]+'</span></div>');$("body").append(divHtml), $(".dShow").find("span").css({
	...
	if(toUser.chatid!="ALL"){
		str='<div style="clear:both; height:0px;"></div><div class="msg'+who+'" id="'+msgid+'" '+msgAuditShow+'><div class="msg_head"><img src="'+User.touxiang+'" class="msg_group_ico" title="'+grouparr[User.color].title+"-"+grouparr[User.color].sn+'"></div><div class="msg_content"><div>'+IsPersonal+'<font class="u" style="color:'+aColor[User.sex]+'" onclick="UserList.menu_in_records(\''+User.chatid+'\',$(this))">'+User.nick+'</font>&nbsp;&nbsp;<font class="dui">对</font> <font class="u" style="color:'+aColor[toUser.sex]+'")">'+toUser.nick+'</font> 说 <font class="date">'+date+'</font></div><div class="layim_chatsay"><font>'+Txt+'</font></div>'+msgBlockBt+msgAuditBt+'</div></div><div style="clear:both; height:0px;"></div>';
		...
		str='<div style="clear:both; height:0px;"></div><div class="msg'+who+'" id="'+msgid+'" '+msgAuditShow+'><div class="msg_head"><img src="'+User.touxiang+'" class="msg_group_ico" title="'+grouparr[User.color].title+"-"+grouparr[User.color].sn+'"></div><div class="msg_content"><div>'+IsPersonal+'<font class="u" style="color:'+aColor[User.sex]+'" onclick="UserList.menu_in_records(\''+User.chatid+'\',$(this))">'+User.nick+'</font>&nbsp;&nbsp;<font class="date">'+date+'</font>';
		...
		str += '</div><div class="layim_chatsay" ><font>'+Txt+'</font></div>'+msgBlockBt+msgAuditBt+'</div></div><div style="clear:both; height:0px;"></div>';
...
function HideMenu()
{
    var elementTable=["ColorTable","Send_key_option","caitiao"];
...
function showplan(txt){
	...
	var s='<div style="clear:both;"></div><div class="msg teacher" ><div class="msg_head"><img src="/face/p1/jihuayuan.jpg"></div><div class="msg_content"><div><font class="u" style="color:#fff" >计划员</font>   <font class="date">&nbsp;&nbsp;'+date+'</font></div> <div class="layim_chatsay"><font style="color:#fff">'+txt+'</font></div></div></div><div style="clear:both;"></div>';
	MsgShow(s,0);
}
function getsysmsg(){
...
function click_automsg(){
   ...
   var msgstrs= new Array();  
    ...
    var Msg='C0MMAND_+_automsg_+_'+rebot_id+'_+_'+'ALL'+'_+_'+msgstrs[cc]+'_+_'+msgid;
   // var msgstr='SendMsg=M=ALL|false|font-size:13px|'+Msg;
     var msgstr = '{"type":"SendMsg","ToChatId":"ALL","IsPersonal":"false","Style":"font-size:13px","tanmu":"'+tanmuSelect+'","device":"computer","Txt":"'+Msg+'"}';
    ws.send(msgstr);
...
function generateCode(){
  	var codeContainer = '<div class="generate-code-container">' + 
  							'<div class="select-code-type">' + 
						        '<select name="getType" id="getType">' + 
						           '<option value="2">砸金蛋</option>' + 
						           '<option value="1">转盘抽奖</option>' + 
						        '</select>' + 
						    '</div>' + 
  							'<div class="generate-copy-code">' + 
  								'<input id="inputCodeSrc" readonly/>' +
  								'<button>复制</button>' + 
    						'</div>' + 
							'<div class="generate-code-btn">' + 
         						'<a href="javascript:void(0)">生成抽奖码</a>' + 
         					'</div>' + 
         		  		'</div>';
	layer.open({
		type: 1,
		...
		shade: false,
		area: ['300px', '200px'],
		content: codeContainer, //iframe的url
		success: function() {
			$('.generate-copy-code button').click(function() {
				$('#inputCodeSrc').select();
				var msg = document.execCommand('Copy') ? '成功' : '失败';
				alert("复制" + msg);				
			});
			$('.generate-code-btn a').click(function() {
				var type = $('#getType')[0].value;
				$.ajax({
					type:'GET',
					dataType: 'JSON',
					url:'../ajax.php?act=getLuckCode&getmethod=' + type,
					success: function(data){
						$('#inputCodeSrc').val(data.luckcode);
					},
