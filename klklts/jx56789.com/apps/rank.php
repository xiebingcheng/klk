...
if($act=="add"){
	if($vuid<1){
        ...
        //admin管理员可以无限制点赞
        if($vuid!=1){
	if($db->num_rows($db->query("select * from {$tablepre}apps_rank where vuid='$vuid' and FROM_UNIXTIME(vtime,'%Y%m%d')='".date('Ymd',gdate())."'"))>0){
		$data["status"]=2;
		$data["msg"]="你今日已经投过他了！";
		exit($json->encode($data));	
	}
...
$query=$db->query("select m.*,ms.*,(select count(*) from  {$tablepre}apps_rank where uid=m.uid and FROM_UNIXTIME(vtime,'%Y%m')='".date("{$om}",gdate())."') as sum,(select count(*) from  {$tablepre}apps_rank where uid=m.uid and FROM_UNIXTIME(vtime,'%Y%m%d')='".date("Ymd",gdate())."') as dsum from {$tablepre}members m,{$tablepre}memberfields ms  where m.uid=ms.uid and m.gid=4 order by sum desc");
...
function calpercent(){
		$('ul li').each(function(){
			...
			var percent= sum ? Math.round(amount/sum*10000)/100: 0;
			percent = percent+'%';
