...
$(function() {
	...
	$("#toRMC").on("click", ".lilist", function() {
		...
		if(id == "cz_10001") {
			...
			window.location.href = cZ.toJWC[5].href;
		}
	})
});
//===================主页方法========================
...
//	yM = "http://m.1681700.com/html/public/home.html";
...
indexObj.ajaxNotice = function() {
	...
	var iferror = false;
	$.ajax({
		url: "/news/index.php/api/index/get_notice",
		type: "GET",
		timeout: 60000,
		async: false,
		success: function(data) {
...
indexObj.loadNotice = function(data) {
	...
	$("#noticeT1").empty();
	$("#noticeT1").text(data.noticeContent);
}

function nofind(obj) {
	obj.src = "../../img/beijmr.png";
}
indexObj.loadBanner = function(data) {
	var data = hometools.ifObj(data); //格式转换
	data = data.result.data;
	var html = "";
	$(".main_image ul").empty();
	...
	$(data).each(function(index) {
		count++;
		var target = "";
		var link = this.link == "" ? "javascript:;" : this.link;
		html += '<a target="_blank" href="' + link + '"><li><img onerror="nofind(this)" src="' + config.imgUrl + '' + this.image + '" /></li></a>';
	});
	$(".main_image ul").append(html);
	if(count<=1){
		$(".flicking_con").hide();
	}
}
...
hometools.hadCode = function(lotCode, type) {
	var pk10 = ["10001", "10012", "10037", "10035"];
...
// $.ajax({
// 	url:  "/news/index.php/api/index/get_adv?type=banner",
// 	type: "GET",
// 	dataType: "json",
// 	timeout: 6e4,
// 	beforeSend: function() {
...
// 			$("#littleimg").empty();
// 			for(var i in t){
// 				$("#littleimg").append('<a target="_blank" href="'+t[i].url+'"><li><img src="'+t[i].img+'" alt=""></li></a>');
// 			}
// 			$(".main_image").touchSlider({
// 				flexible: true,
// 				speed: 200,
