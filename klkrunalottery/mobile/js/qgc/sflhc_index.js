...
method.indexLoad = function(id) {
	...
	var id = "#" + $(id).attr("id");
	var data = {
		url: '/news/index.php?m=index&c=index&a=kaijiang&type=getLotteryInfo',
		id:boxId,
		lotCode:lotCode
	};
	if(config.ifdebug){
...
method.listData = function(date) {
		$.ajax({
			url: '/news/index.php?m=index&c=index&a=kaijiang&type=getHistoryLotteryInfo&date=' + date,
			type: "GET",
			data: {
				"lotCode": lotCode
			},
